...
QtEditor::QtEditor(QWidget *parent) : QMainWindow(parent) {
    ...
    setCentralWidget(textedit); //중앙 위젯으로 설정
...
/* ****************************************** [ Menu Bar ] *************************************** */
    ...
    /* 2.메소드, Quit Menu */
    QAction *quitAct = makeActionMethod("quit.png", tr("&Quit"), tr("Ctrl+Q"), tr("quit this Program"), this, SLOT(quitFile()));
    QAction *undoAct = makeActionMethod("undo.png", tr("&Undo"), tr("Ctrl+Z"), tr("Undo"), this, SLOT(undoFile()));
    /* 3. 템플릿,  */
    /* 4. 람다함수,  */
    ...
    fileMenu->addAction(undoAct);                   //메뉴에 서브메뉴(open) 등록
